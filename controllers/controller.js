...
export async function generateOTP(req, res) {
    ...
    let isPrev = await OTP.findOne({username: req.query.username});
    if (isPrev) await OTP.updateOne({username: req.query.username}, {$set: {code: otp}});
    else await new OTP({username: req.query.username, code: otp}).save();
    console.log(otp);
    
    return res.status(201).send({ "Success": true, code: otp });
}
